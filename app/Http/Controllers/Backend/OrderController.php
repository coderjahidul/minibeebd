...
class OrderController extends Controller
{
    ...
    public function getOrderProduct(Request $request)
    {
        ...
        $data = Variation::query()
            ...
            ->select(
                'variations.id',
                DB::raw("TRIM(CONCAT(products.name,' (',COALESCE(sizes.title,''),'),(',COALESCE(colors.name,''),')')) AS value")
            )
    ...
    public function orderProductEntry(Request $request)
    {
        ...
        $sizeModel = $variation->relationLoaded('size')
            ? $variation->getRelation('size')
            : $variation->size()->first();
        $colorModel = $variation->relationLoaded('color')
            ? $variation->getRelation('color')
            : $variation->color()->first();
        $sizeTitle = $sizeModel ? (string) ($sizeModel->title ?? $sizeModel->name ?? '') : '';
        $colorName = $colorModel ? (string) ($colorModel->name ?? $colorModel->title ?? '') : '';
